from datetime import UTC, datetime
from unittest.mock import AsyncMock, MagicMock
from uuid import UUID

from fastapi.testclient import TestClient
from sqlalchemy.ext.asyncio import AsyncSession

from webhook_delivery_service.database import get_database_session
from webhook_delivery_service.main import app
from webhook_delivery_service.models import WebhookDelivery

client = TestClient(app)


def test_get_delivery_returns_existing_delivery() -> None:
    delivery_id = UUID("12345678-1234-5678-1234-567812345679")
    created_at = datetime(2026, 8, 16, 15, 0, tzinfo=UTC)

    expected_delivery = WebhookDelivery(
        id=delivery_id,
        target_url="https://example.com/webhooks",
        event_type="order.created",
        payload={"order_id": 123},
        status="pending",
        attempt_count=0,
        next_attempt_at=None,
        last_error=None,
        created_at=created_at,
        delivered_at=None,
    )

    session = MagicMock(spec=AsyncSession)
    session.get = AsyncMock(return_value=expected_delivery)

    async def override_database_session():
        yield session

    app.dependency_overrides[get_database_session] = override_database_session

    try:
        response = client.get(f"/deliveries/{delivery_id}")
    finally:
        app.dependency_overrides.pop(get_database_session, None)

    assert response.status_code == 200
    assert response.json() == {
        "id": str(delivery_id),
        "target_url": "https://example.com/webhooks",
        "event_type": "order.created",
        "payload": {"order_id": 123},
        "status": "pending",
        "attempt_count": 0,
        "next_attempt_at": None,
        "last_error": None,
        "created_at": "2026-08-16T15:00:00Z",
        "delivered_at": None,
    }

    session.get.assert_awaited_once_with(
        WebhookDelivery,
        delivery_id,
    )


def test_get_delivery_returns_not_found() -> None:
    delivery_id = UUID("12345678-1234-5678-1234-567812345680")

    session = MagicMock(spec=AsyncSession)
    session.get = AsyncMock(return_value=None)

    async def override_database_session():
        yield session

    app.dependency_overrides[get_database_session] = override_database_session

    try:
        response = client.get(f"/deliveries/{delivery_id}")
    finally:
        app.dependency_overrides.pop(get_database_session, None)

    assert response.status_code == 404
    assert response.json() == {
        "detail": "Webhook delivery not found",
    }

    session.get.assert_awaited_once_with(
        WebhookDelivery,
        delivery_id,
    )
